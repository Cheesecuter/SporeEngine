...
namespace Spore
{
	Mesh::Mesh(std::vector<Vertex> vertices_p, std::vector<uint32> indices_p, std::vector<Texture> textures_p) :
		vertices(vertices_p), indices(indices_p), textures(textures_p)
	{
		SetupMesh();
	}

	void Mesh::Draw(Shader& shader_p)
	{
		...
		for (uint32 i = 0; i < textures.size(); i++)
		{
			...
			glUniform1i(glGetUniformLocation(shader_p.ID, (name + number).c_str()), i);
